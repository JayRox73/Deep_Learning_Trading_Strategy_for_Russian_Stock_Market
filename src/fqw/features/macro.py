...
def add_macro_features(df_ticker: pd.DataFrame, data_dir: Path | str = "data") -> pd.DataFrame:
    ...
    try:
        ...
        moex_daily = (
            df_moex.groupby("Date").agg({"Close": "last"}).rename(columns={"Close": "MOEX_Close"})
        )
        ...
        moex_daily[["MOEX_Close", "MOEX_Vol_20d"]] = (
            moex_daily[["MOEX_Close", "MOEX_Vol_20d"]].shift(1).ffill()
        )
    ...
    df = pd.merge_asof(
        df, key_rate_sorted, left_on="DateTime", right_on="Date", direction="backward"
    )
